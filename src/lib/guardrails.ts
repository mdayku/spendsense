...
export function enforceConsent(consent: { status: "OPTED_IN" | "OPTED_OUT" } | null) { if (!consent || consent.status !== "OPTED_IN") throw new Error("ConsentRequired: user has not opted in."); }
